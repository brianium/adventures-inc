...
// this is a copy of the extra credit db but attemps to use a global connection struct
...
// define a global conntection variable
...
void Database_create(int max_size, int max_rows)
{
    int i = 0;
...
void Database_find(char field, const char *value)
{
    int i = 0;
    struct Database *db = conn->db;
    switch (field) {
        ...
        default:
            die("Cannot search on given field");
    }
}
...
void Connection_check_id(int id)
{
    struct Database *db = conn->db;
    if (id >= db->max_rows) {
...
int main(int argc, char *argv[])
{
    ...
    switch(action) {
        case 'c':
            ...
            Database_create(max_size, max_rows);
        ...
        case 'f':
            if (argc != 5) die("Find requires a search key and value");
            searchField = argv[3][0];
            Database_find(searchField, argv[4]);
            break;
